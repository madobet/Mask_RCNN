...
if [ "${GPU_BRAND#*[Nn][Vv][Ii][Dd][Ii][Aa]}" != "$GPU_BRAND" ]; then
    ...
    sudo docker build -f tfhub_nvidia_v1.Dockerfile --tag tfhub_v1:nvidia-$VERSION .
    sudo docker run -it --rm --gpus all -v $HOME:/home/tfhub -p 8000:8000 tfhub_v1:nvidia-$VERSION
    ...
    # sudo docker build -f tfhub_v1_amd.Dockerfile --tag tfhub_v1:amd-$VERSION .
    # sudo docker run -it --rm --gpus all -v $HOME:/home/tfhub -p 8000:8000 tfhub_v1:amd-$VERSION
fi
